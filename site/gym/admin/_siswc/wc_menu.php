...
<?php if (APP_SERVICES && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_SERVICES):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_services_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<!-- CLASSES -->
<?php if (APP_CLASSES && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_CLASSES):
    $wc_classes_alerts = null;
    $Read->FullRead("SELECT count(class_id) as total FROM " . DB_CLASSES . " WHERE class_status != 1");
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_classes_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
    <li class="dashboard_nav_menu_li <?= strstr($getViewInput, 'classes/') ? 'dashboard_nav_menu_active' : ''; ?>"><a class="icon-power" title="Classes" href="dashboard.php?wc=classes/home">Classes<?= $wc_classes_alerts; ?></a>
        <ul class="dashboard_nav_menu_sub">
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'classes/home' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Ver Classes" href="dashboard.php?wc=classes/home">&raquo; Ver Classes</a></li>
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'classes/create' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Nova Classe" href="dashboard.php?wc=classes/create">&raquo; Nova Classe</a></li>
        </ul>
    </li>
<?php endif; ?>

<!-- TREINADORES -->
<?php if (APP_TRAINEES && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_TRAINEES): ?>
    <li class="dashboard_nav_menu_li <?= strstr($getViewInput, 'treinadores/') ? 'dashboard_nav_menu_active' : ''; ?>"><a class="icon-user" title="Treinadores" href="dashboard.php?wc=treinadores/home">Treinadores</a>
        <ul class="dashboard_nav_menu_sub">
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'treinadores/home' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Ver Treinadores" href="dashboard.php?wc=treinadores/home">&raquo; Ver Treinadores</a></li>
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'treinadores/create' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Novo Treinador" href="dashboard.php?wc=treinadores/create">&raquo; Novo Treinador</a></li>
        </ul>
    </li>
<?php endif; ?>

<!-- PLANOS -->
<?php if (APP_PLANS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_PLANS): ?>
    <li class="dashboard_nav_menu_li <?= strstr($getViewInput, 'planos/') ? 'dashboard_nav_menu_active' : ''; ?>"><a class="icon-credit-card" title="Planos" href="dashboard.php?wc=planos/home">Planos</a>
        <ul class="dashboard_nav_menu_sub">
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'planos/home' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Ver Planos" href="dashboard.php?wc=planos/home">&raquo; Ver Planos</a></li>
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'planos/create' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Novo Plano" href="dashboard.php?wc=planos/create">&raquo; Novo Plano</a></li>
        </ul>
    </li>
<?php endif; ?>

<!-- OFERTAS -->
<?php if (APP_OFFERS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_OFFERS): ?>
    <li class="dashboard_nav_menu_li <?= strstr($getViewInput, 'ofertas/') ? 'dashboard_nav_menu_active' : ''; ?>"><a class="icon-cart" title="Ofertas" href="dashboard.php?wc=ofertas/home">Ofertas</a>
        <ul class="dashboard_nav_menu_sub">
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'ofertas/home' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Ver Ofertas" href="dashboard.php?wc=ofertas/home">&raquo; Ver Ofertas</a></li>
            <li class="dashboard_nav_menu_sub_li <?= $getViewInput == 'ofertas/create' ? 'dashboard_nav_menu_active' : ''; ?>"><a title="Novo Oferta" href="dashboard.php?wc=ofertas/create">&raquo; Novo Oferta</a></li>
        </ul>
    </li>
<?php endif; ?>
...
<?php if (APP_SCHEDULES && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_SCHEDULES):
    ...
    $Read->FullRead("SELECT count(schedule_id) as total FROM " . DB_SCHEDULES . " WHERE schedule_status = 1");
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_schedules_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
    <li class="dashboard_nav_menu_li <?= strstr($getViewInput, 'agendamentos/') ? 'dashboard_nav_menu_active' : ''; ?>"><a class="icon-calendar" title="Agendamentos" href="dashboard.php?wc=agendamentos/home">Agendamentos <?= $wc_schedules_alerts; ?></a></li>
<?php endif; ?>
...
<?php if (APP_POSTS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_POSTS):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_posts_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if (APP_PAGES && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_PAGES):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_pages_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if (APP_COMMENTS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_COMMENTS):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_comment_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if (APP_EAD):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_ead_courses_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;

    $wc_ead_support_alerts = null;
    $Read->FullRead("SELECT count(support_id) as total FROM " . DB_EAD_SUPPORT . " WHERE support_status = 1");
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_ead_support_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;

    $wc_ead_orders_alerts = null;
    $Read->FullRead("SELECT count(order_id) as total FROM " . DB_EAD_ORDERS . " WHERE order_status = 'chargeback'");
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_ead_orders_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
...
<?php if (APP_PRODUCTS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_PRODUCTS):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_pdt_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if (APP_ORDERS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_PRODUCTS_ORDERS):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_order_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if (APP_IMOBI && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_IMOBI):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_imobi_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if (APP_TESTIMONIALS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_TESTIMONIALS):
    ?>
    <li class="dashboard_nav_menu_li <?= strstr($getViewInput, 'depoimentos/') ? 'dashboard_nav_menu_active' : ''; ?>"><a class="icon-bubbles3" title="Depoimentos" href="dashboard.php?wc=depoimentos/home">Depoimentos</a></li>
<?php endif; ?>
...
<?php if (APP_SLIDE && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_SLIDES):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_slide_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if (APP_GALLERY && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_GALLERY):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_gallery_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if(APP_VIDEOS && $_SESSION['userLogin']['user_level'] >= LEVEL_WC_VIDEOS):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_videos_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if ($_SESSION['userLogin']['user_level'] >= LEVEL_WC_HELLO):
    ...
    if ($Read->getResult() && $Read->getResult()[0]['total'] >= 1):
        $wc_hello_alerts .= "<span class='wc_alert bar_green'>{$Read->getResult()[0]['total']}</span>";
    endif;
    ?>
...
<?php if ($_SESSION['userLogin']['user_level'] >= LEVEL_WC_REPORTS):
    ?>
    <li class="dashboard_nav_menu_li <?= strstr($getViewInput, 'report') ? 'dashboard_nav_menu_active' : ''; ?>"><a class="icon-pie-chart" title="Relatórios" href="dashboard.php?wc=reports/home">Relatórios</a>
        <ul class="dashboard_nav_menu_sub">
            <?php if (APP_EAD): ?>
