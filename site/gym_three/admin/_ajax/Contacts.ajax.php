        ...
        case 'content':
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $CONTACTS):
                    ...
                    $Name = (!empty($contact_name) ? Check::Chars($contact_name, 30) : 'Não Informado' );
                    $Phone = (!empty($contact_telephone) ? $contact_telephone : 'Não Informado' );
                    
                    $jSON['content'] .= "<article class='marketing__table js-marketing-table js-rel-to' id='{$contact_id}'> <div class='marketing__data'> <p class='icon-user'>{$Name}</p> <p class='icon-envelop'>" . Check::Chars($contact_email, 30) . "</p> <p class='icon-phone'>{$Phone}</p> <p> <span title='Excluir Contato' class='j_delete_action icon-bin icon-notext btn_header btn_red' callback='Contacts' callback_action='delete' id='{$contact_id}'></span> </p> </div> </article>";
                endforeach;
            endif;
            break;    
