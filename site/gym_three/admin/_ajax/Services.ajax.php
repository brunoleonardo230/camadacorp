    ...
    switch ($Case):
        ...
        case 'manager':
            ...
            if (!empty($_FILES['service_image'])):
                ...
                $Upload->Image($File, $PostData['service_name']. '-image', IMAGE_W, 'services');
                if ($Upload->getResult()):
            ...
            if (!empty($_FILES['service_icon'])):
                ...
                $Upload = new Upload('../../uploads/');
                 $Upload->Image($File, $PostData['service_name']. '-icon', IMAGE_W, 'services');
                if ($Upload->getResult()):
        ...
        case 'service_images':
            ...
            $ServiceName = (!empty($PostData['service_name']) ? $PostData['service_name'] : $ThisServices['service_title']);
            $PostData['service_name'] = Check::Name($ServiceName);
            $Read->FullRead("SELECT service_name FROM " . DB_SERVICES . " WHERE service_name = :nm AND service_id != :id", "nm={$PostData['service_name']}&id={$ServiceId}");
            if ($Read->getResult()):
                $PostData['service_name'] = "{$PostData['service_name']}-{$ServiceId}";
            ...
            $jSON['name'] = $PostData['service_name'];	
                if (!empty($_FILES['service_image_one'])):
                $File = $_FILES['service_image_one'];

                if ($ThisServices['service_image_one'] && file_exists("../../uploads/{$ThisServices['service_image_one']}") && !is_dir("../../uploads/{$ThisServices['service_image_one']}")):
                        unlink("../../uploads/{$ThisServices['service_image_one']}");
                endif;

                $Upload = new Upload('../../uploads/');
                $Upload->Image($File, $PostData['service_name'] . '-one', IMAGE_W, 'services');
                if ($Upload->getResult()):
                        $PostData['service_image_one'] = $Upload->getResult();
                else:
                ...
                unset($PostData['service_image_one']);
            endif;
                if (!empty($_FILES['service_image_two'])):
                $File = $_FILES['service_image_two'];

                if ($ThisServices['service_image_two'] && file_exists("../../uploads/{$ThisServices['service_image_two']}") && !is_dir("../../uploads/{$ThisServices['service_image_two']}")):
                    unlink("../../uploads/{$ThisServices['service_image_two']}");
                endif;

                $Upload = new Upload('../../uploads/');
                 $Upload->Image($File, $PostData['service_name'] . '-two', IMAGE_W, 'services');
                if ($Upload->getResult()):
                    $PostData['service_image_two'] = $Upload->getResult();
                else:
                ...
                unset($PostData['service_image_two']);
            endif;

            $Update->ExeUpdate(DB_SERVICES, $PostData, "WHERE service_id = :id", "id={$ServiceId}");
            $jSON['alert'] = ["green", "checkmark", "TUDO CERTO {$_SESSION['userLogin']['user_name']}", "As Imagens do Serviço Foram Atualizadas Com Sucesso!"];
            break;
        ...
        case 'delete':
            ...
            $Read->FullRead("SELECT service_image_one FROM " . DB_SERVICES . " WHERE service_id = :ps", "ps={$PostData['service_id']}");
            if ($Read->getResult() && file_exists("../../uploads/{$Read->getResult()[0]['service_image_one']}") && !is_dir("../../uploads/{$Read->getResult()[0]['service_image_one']}")):
                unlink("../../uploads/{$Read->getResult()[0]['service_image_one']}");
            endif;
			
            $Read->FullRead("SELECT service_image_two FROM " . DB_SERVICES . " WHERE service_id = :ps", "ps={$PostData['service_id']}");
            if ($Read->getResult() && file_exists("../../uploads/{$Read->getResult()[0]['service_image_two']}") && !is_dir("../../uploads/{$Read->getResult()[0]['service_image_two']}")):
                unlink("../../uploads/{$Read->getResult()[0]['service_image_two']}");
            endif;
